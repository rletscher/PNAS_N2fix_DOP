...
# Model 2: Iron limitation is function of dissolved [Fe] : NO3 supply stoichiometry
...
function diaz_DIP(x, p)
    @unpack αd, β = p
    t_val = t(p)
     @. αd * (p2c_v * NPP_v / NPP0 / (WOAPO4_v / DIP0))^β * x * FeN_v_norm * t_val * (z_top ≤ 73)
end
...
jldsave("output_Model2_20.jld2"; DIP=DIP, DOP=DOP, POP=POP, Jfix=Jfix, tp_opt=tp_opt, hess=hess)

matwrite("pooloutput_Model2_20.mat", Dict(
