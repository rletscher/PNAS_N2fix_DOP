...
# Model 2: Iron limitation is function of dissolved [Fe] : NO3 supply stoichiometry & diazotroph DIP uptake is function of P*
...
# compute Fe:N using column integrals over z levels 1:3
Fe_col   = sum(Fe[:,:,1:3],   dims=3)
NO3_col  = sum(NO3_obs[:,:,1:3], dims=3)
FeN_2D = dropdims(Fe_col ./ NO3_col, dims=3)
FeN_2D = replace!(FeN_2D, NaN => 1e-6)

FeN_norm = (FeN_2D .- minimum(FeN_2D)) ./ (maximum(FeN_2D) .- minimum(FeN_2D))     # 0-1 normalization

# copy normalized 2D values into levels 1 and 2 of the full 3D FeN array
FeN_large = Fe ./ NO3_obs
FeN_large[:,:,1] .= FeN_norm
FeN_large[:,:,2] .= FeN_norm

FeN_v_norm = vectorize(FeN_large, grd)
FeN_v_norm = replace!(FeN_v_norm, NaN => 1e-6)
for i in eachindex(FeN_v_norm)
    if FeN_v_norm[i] < 0
        FeN_v_norm[i] = 1e-6
    end
end
...
function diaz_SLDOP(y, p)
     @unpack Ko = p
     t_val = t(p)
      @. ((Ko+WOAPO4_v) / (WOAPO4_v + 0.000001))/ Vm * y * FeN_v_norm * t_val * (z_top≤73)
end
...
function diaz_DIP(x, p)
    @unpack αd, β = p
    t_val = t(p)
     Pstar_v = x .- WOANO3_v ./ n2p_v
     Pstar_v = max.(Pstar_v, 0.0)                   # only where P* is positive
     Pstar_v = min.(Pstar_v, x)                     # P* can't be > DIP
     @. αd * (p2c_v * NPP_v / NPP0 / (WOAPO4_v / DIP0))^β * Pstar_v * FeN_v_norm * t_val * (z_top ≤ 73)
end
...
jldsave("output_Model4_20.jld2"; DIP=DIP, DOP=DOP, POP=POP, Jfix=Jfix, tp_opt=tp_opt, hess=hess)

matwrite("pooloutput_Model4_20.mat", Dict(
